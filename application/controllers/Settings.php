...
public function managedepartment() {//Tamang(Managing department)
	...
	public function postnews(){
		...
		{
			$des=$this->input->post('describtion');

			$this->news_model->savenews($des);
			echo 'alert("the news post successfully");';
            redirect('Settings/postnews');
	...
	public function displaydata(){
		$result['data']=$this->news_model->displayrecord();
	   $this->load->view('alumni',$result);
	}

	

	public function deletenews(){//gagen
		$this->load->view('template/includeheader',$this->dataheader);
		$result['data']=$this->news_model->display_records();
		$this->load->view('delete_news',$result);
		
		$this->load->view('template/includefooter');
	}
	public function deletedata()//gagen
	{
	
	$id=$this->input->get('id');
	$this->news_model->deleterecords($id);
	redirect('Settings/deletenews');
	}
	...
	public function updateDetail(){
	 ...
	 public function  registered_user(){
	 	$this->load->view('template/includeheader',$this->dataheader);
	 	$data['request']=$this->db->query("SELECT * from bpas_logins where status1 = 'requestp'")->result_array();

       		$this->load->view('registered_user',$data);

       		
       		//$data['item']=$this->db->get('cst_stocktable1')->result_array();
       		
       		$this->load->view('template/includefooter');
	 }

	  public function  registered_user1(){//Tamang(message for registered user)
	  	$this->registered_user();
	  }

	   public function  notregistered_user1(){//Tamang(message for registered user)
	  	$this->registered_user();
	  }

	public function rejected($param1=''){//Tamang(When clicking the reject button)
	 	
        	$dat['status1']='pending';
        	// $dat['forwardStatus']='Y';
		 	$this->db->where('loginId', $param1);
	        $this->db->update('bpas_logins', $dat);
	        if($this->db->affected_rows()>0){
			
	 $data1['message']='<br /><br /><br /><span class="alert alert-info">You have successfully rejected</span> <br /><br /><br />';
			 	
			$this->load->view('userManagement/acknowledgemntwithoutheaderfooter1',$data1);
        }
        else{
			$data1['message']='<br /><br /><br /><span class="alert alert-info">Not rejected</span> <br /><br /><br />';
...
function addDak($param="")
{
	...
	public function get_dropdownlistforuser() {//Tamang(dropdown list for user )
		...
		echo "<tr>";
		echo "<td>$counter</td>";
		 echo"<td><input type='checkbox' class='delete_checkbox' value='$row->relatedUserId' /></td>";
	...
	function delete_all()//(Tamang)Deleting using checkbox
 {
                ...
                $data = array(  
                 ...
                 $data1 = array(  
                	...
                	"AgencyParentID"     =>$this->input->post("parent"),
                	 "FirstName"     =>$this->input->post("FirstName"),  
                	 "gender"     =>$this->input->post("gender"),
                	"department"     =>$this->input->post("gender1"),
                	"batch"     =>$this->input->post("gender2"),
					"AgencyID"     =>$this->input->post("agency"),
